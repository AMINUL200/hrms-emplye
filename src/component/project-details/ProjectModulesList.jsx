...
const filterTree = (items = [], search = "", statusFilter = "All Status") => {
  ...
  return items
    .map((item) => {
      ...
      const matchesSearch =
        ...
        item.unique_id?.toLowerCase().includes(normalizedSearch);
...
const ProjectModulesList = ({ modulesInfo = [] }) => {
  ...
  useEffect(() => {
    if (!Array.isArray(modulesInfo) || modulesInfo.length === 0) {
      return;
    }
  ...
  const filteredModules = useMemo(() => {
    return filterTree(modulesInfo, search, statusFilter);
  }, [modulesInfo, search, statusFilter]);
  ...
  const handleDropdownAction = (action, item) => {
    ...
    if (action === "View") {
      // Construct the URL
      const url = `/organization/assigned-project/${item.project_id}/workspace/${item.id}`;
    ...
    switch (item.type) {
        ...
        return (
          <div className="pml-right-info">
            <span className="pml-info-item">
              ...
              <span className="pml-info-value">{item.total_tasks ?? 0}</span>
            </span>
        ...
        return (
          <div className="pml-right-info">
            ...
            <span className="pml-info-item">
              ...
              <span className="pml-info-value">{item.total_subtasks ?? 0}</span>
            </span>
          </div>
        );
      ...
      case "subtask":
        return (
          <div className="pml-right-info">
            <span className="pml-info-item">
              <span className="pml-info-value">N/A -</span>
            </span>
          </div>
        );
  ...
  const renderTree = (items = [], level = 0, parentIndex = "") => {
    return items.map((item, itemIndex) => {
      ...
      const indent = getIndentLevel(level);

      const isDropdownOpen = openDropdown === item.id;

      const progress = item.assignment_summary?.percentage ?? 0;
        ...
        <div className="pml-module-group" key={item.id}>
          ...
          >
            ...
            <span
              className={`
                  pml-row-title
                  ${item.type === "subtask" ? "pml-row-title--subtask" : ""}
                `}
              title={item.title}
            >
              {item.title}
            ...
            <div className="pml-progress-wrap">
              ...
              <span className="pml-progress-pct">{progress}%</span>
            </div>
          ...
          {hasChildren && isOpen && (
            <div className="pml-children">
              {renderTree(item.children, level + 1, displayIndex)}
            </div>
          )}
        </div>
      );
    ...
    <div className="pml-card">
      ...
      <div className="pml-header">
        <div className="pml-title">
          ...
          <span className="pml-count-badge">{modulesInfo.length}</span>
        </div>
