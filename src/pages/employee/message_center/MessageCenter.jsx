...
const avatarColors = [
    '#FF902F', '#667eea', '#764ba2', '#4CAF50', '#2196F3',
...
const MessageCenter = () => {
            ...
            try {
                ...
                if (response.data.status === 200) {
                    const projectsData = response.data.projects.map((project, index) => {
                        ...
                        const lastMessage = project.messages?.length > 0 ? project.messages[project.messages.length - 1] : null;
                        ...
                        return {
                            ...
                            lastMessage: lastMessage 
                                ? `${lastMessage.employee_code === data?.employee_id ? 'You' : lastMessage.user_name?.split(' ')[0]}: ${lastMessage.message?.substring(0, 30)}${lastMessage.message?.length > 30 ? '...' : ''}`
                                : 'No messages yet',
    ...
    const handleProjectSelect = (project) => {
        ...
        // If mobile view, show chat section
        if (isMobileView) {
            setShowChat(true);
        }
    };
    ...
    const handleSendMessage = async () => {
        if ((!messageText.trim() && attachments.length === 0) || !selectedProject) return;
    ...
    const refreshProjectMessages = async (projectId) => {
        try {
            const response = await axios.get(`${api_url}/projects/members/${projectId}`, {
                headers: { Authorization: `Bearer ${token}` },
                params: { t: Date.now() } // Prevent caching
            });
                    ...
                    if (project.project_id === projectId) {
                        ...
                        return {
                            ...
                            lastMessage: lastMessage 
                                ? `${lastMessage.employee_code === data?.employee_id ? 'You' : lastMessage.user_name?.split(' ')[0]}: ${lastMessage.message?.substring(0, 30)}${lastMessage.message?.length > 30 ? '...' : ''}`
                                : project.lastMessage,
                            timestamp: lastMessage ? getTimeAgo(lastMessage.created_at) : project.timestamp
                        };
                    }
                    return project;
                }));
    ...
    const getCurrentUserRole = () => {
        ...
        const member = selectedProject.members.find(m => m.emp_code === data.employee_id);
    ...
    const getInputPlaceholder = () => {
        ...
        if (replyingTo) return `Reply to ${replyingTo.user_name || replyingTo.employee_code}...`;
    ...
    return (
        <div className="message-center-container">
            ...
            {showProjectDetails && selectedProject?.type === 'group' && (
                <div className="group-details-overlay" onClick={() => setShowProjectDetails(false)}>
                    <div className="group-details-modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h3>Project Information</h3>
                            <button className="close-btn" onClick={() => setShowProjectDetails(false)}>
                        ...
                        <div className="modal-content">
                            <div className="group-avatar-large" style={{ backgroundColor: selectedProject.avatarColor }}>
                            ...
                            <div className="group-members-count">{selectedProject.members.length} members</div>
                                    ...
                                    <div key={index} className="member-item">
                                        ...
                                        <div className="member-info">
                                            <div className="member-name">
                                                ...
                                                {member.emp_code === data?.employee_id && <span className="you-badge">You</span>}
                                            </div>
            ...
            <div className={`message-sidebar ${isMobileView && showChat ? 'hidden' : ''}`}>
                <div className="sidebar-header">
                    ...
                    <button className="new-chat-btn" title="Refresh" onClick={() => window.location.reload()}>
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <>
                        ...
                        <div className="chat-header">
                                    ...
                                    <div className="chat-user-status">
                                        <div className="group-members-preview">
                                            ...
                                            {selectedProject.members.length > 3 && (
                                                <span className="more-members">+{selectedProject.members.length - 3} more</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="chat-actions">
                                <div className="action-icon" onClick={() => refreshProjectMessages(selectedProject.project_id)}>
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                        <path d="M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15" />
                                    </svg>
                                </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="message-content-wrapper">
                                                    ...
                                                    <div className="message-text">{message.message || message.title}</div>
                                                    {message.file && (
                                                        <div className="message-attachment">
                                                            <div 
                                                                className="attachment-preview"
                                                                onClick={() => window.open(`${stor_url}/${message.file}`, '_blank')}
                                                            >
                                                        ...
                                                        <div className="message-replies">
                                                            {message.replies.map(reply => (
                                                                <div key={reply.id} className="reply-item">
                                                                    ...
                                                                    <div className="reply-text">{reply.message || reply.title}</div>
                        ...
                        <div className="message-input-container">
                                ...
                                <div className="action-indicator">
                                    <div className="action-info">
                                        {editingMessage ? (
                                            ...
                                            <>
                                                <span className="action-icon">↩️</span>
                                                <span>Replying to {replyingTo.user_name || replyingTo.employee_code}</span>
                                            </>
                                        )}
                                    </div>
                                    <button className="cancel-action-btn" onClick={handleCancelAction}>
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                            <path d="M18 6L6 18M6 6l12 12" />
                                        </svg>
                                    </button>
                            ...
                            <div className="message-input">
                                ...
                                />
                                <div className="input-attachment" onClick={() => setShowAttachmentPopup(!showAttachmentPopup)}>
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
